...
resource "aws_route53_record" "abpt_ecs_cert_validation_record" {
    name                            = "${aws_acm_certificate.arca-blanca-domaincert.domain_validation_options.0.resource_record_name}" 
    type                            = "${aws_acm_certificate.arca-blanca-domaincert.domain_validation_options.0.resource_record_type}"
    zone_id                         = "${data.aws_route53.arca-blanca-ecsdomain.zone_id}" 
    records                         = ["${aws_acm_certificate.arca-blanca-domaincert.domain_validation_options.0.resource_record_value}"]
    ttl                             = 60
}
